...
public abstract class SpriteRoot : MonoBehaviour, IEZLinkedListItem<ISpriteAnimatable>, IUseCamera
{
	...
	*/
	...
	public EZScreenPlacement ScreenPlacer {
		get { return screenPlacer; }
	}
	...
	protected virtual void Awake ()
	{
		...
		if (!managed) {
			...
			if (manager != null) {
				...
				ScreenLog.AddMessage("Managed sprite \"" + name + "\" has not been assigned to a SpriteManager!", ScreenLogType.Error);
		}
		...
		//Pat sets this manually so we can get pixel dimensions soon
	...
	public virtual void Start ()
	{
		...
		ResizeAndReposition (this);

		if (hideAtStart)
			Hide (true);
		
		//Invoke ("ResizeAgain", 0.01f); needed this for web players sometimes for some reason
	}
	
	#region PatEdits
	
	//call after init just to be sure
	void ResizeAgain()
	{
		ResizeAndReposition(null);
	}
	
	[HideInInspector]
	public SpriteRoot parentSprite;
	
	[HideInInspector]
	public bool keepOriginalParent = false;
	
	[HideInInspector]
	public ResizeRepositionOptions resizeOptions;
	[HideInInspector]
	public Vector2 resizePercentage;
	
	[HideInInspector]
	public ResizeRepositionOptions repositionOptions;
	[HideInInspector]
	public Vector2 repositionPercentage;
	
	private bool localyResizedAndRepositioned = false;
	
	/// <summary>
	/// Resizes and Repositions based on percentages
	/// </summary>
	/// <param name='sprite'>
	/// Sprite.
	/// </param>
	public void ResizeAndReposition (SpriteRoot spriteCalledFrom)
	{
		...
		if (this.Equals (spriteCalledFrom) == true)
		{
			if (localyResizedAndRepositioned == true)
				return;
			else
				localyResizedAndRepositioned = true;
		}
		
		if (resizeOptions == ResizeRepositionOptions.none && repositionOptions == ResizeRepositionOptions.none)
			return;
		...
		switch (resizeOptions)
		{
			case ResizeRepositionOptions.byScreenPercentage:
				...
				PixelSize = new Vector2 (renderCamera.pixelWidth * (resizePercentage.x / 100), renderCamera.pixelHeight * (resizePercentage.y / 100));
			...
			case ResizeRepositionOptions.bySpritePercentage:
				...
				if (parentSprite != null)
					SetSize (parentSprite.width * (resizePercentage.x / 100), parentSprite.height * (resizePercentage.y / 100));
				else
		...
		switch (repositionOptions)
		{
			...
			case ResizeRepositionOptions.bySpritePercentage:
				...
				if (parentSprite != null)
				{
					...
					transform.localPosition = new Vector3 (temp.x, temp.y, transform.localPosition.z); // allows u to change z in inspector
		...
		gameObject.BroadcastMessage("ResizeAndReposition", this, SendMessageOptions.DontRequireReceiver);
	}
...
// 			return true;
	...
	public Vector2 PixelSize {
		get {
			//using * gave me errors, / gave me correct values
			Vector2 returnVal = new Vector2 (width / worldUnitsPerScreenPixel, height / worldUnitsPerScreenPixel);
			return returnVal;
		}
		set {
