...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	public List<IUIListObject> Items {
		get{ return items; }	
	}
	...
	{
		return new Vector3 (-viewableArea.x / 2, viewableArea.y / 2, transform.position.z);
	}
	...
	public void ResizeBCToViewableArea ()
	{
		BoxCollider col = GetComponent(typeof(BoxCollider)) as BoxCollider;
		
		if (col != null)
			col.size = new Vector3 (viewableArea.x, viewableArea.y, col.size.z);
	}
	...
	public void AddItem(GameObject itemGO)
	{
		...
		if (item == null)
		{
			ScreenLog.AddMessage("GameObject \"" + itemGO.name + "\" does not contain any list item component suitable to be added to scroll list \"" + name + "\".", ScreenLogType.Warning);
			return;
		}
