...
public class EZScreenPlacement : MonoBehaviour, IUseCamera
{
	...
	public void Start()
	{
		...
		{
			EZScreenPlacement c = relativeObject.GetComponent(typeof(EZScreenPlacement)) as EZScreenPlacement;
			if (c != null)
				c.AddDependent(this);
		}
    ...
    public Vector3 ScreenPosToWorldPos(Vector3 screenPos) 
	{
		...
		if (renderCamera == null)
		{
			ScreenLog.AddMessage("Render camera not yet assigned to EZScreenPlacement component of \"" + name + "\" when attempting to call PositionOnScreen()", ScreenLogType.Error);
...
public class EZScreenPlacementMirror
{
	...
	public virtual bool Validate(EZScreenPlacement sp)
	{
		...
		if (sp.relativeObject != null)
		{
			...
			{
				ScreenLog.AddMessage("ERROR: The Relative Object you recently assigned on \"" + sp.name + "\" which points to \"" + sp.relativeObject.name + "\" would create a circular dependency.  Please check your placement dependencies to resolve this.", ScreenLogType.Error);
				sp.relativeObject = null;
			}
		}
	...
	public virtual bool DidChange(EZScreenPlacement sp)
	{
		...
		if (relativeObject != sp.relativeObject)
		{
			...
			if(sp.relativeObject != null)
			{
				EZScreenPlacement c = sp.relativeObject.GetComponent(typeof(EZScreenPlacement)) as EZScreenPlacement;
				if (c != null)
					c.AddDependent(sp);
			}
#endif
