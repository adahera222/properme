...
{
	public string atlasFolder = "Textures/SpriteAtlases";	// Folder in which to place generated atlases
	...
	void OnWizardCreate()
	{
		...
		for (int i = 0; i < matSpriteMap.Count; ++i)
		{
			...
			{
				PatStuff.ScreenLog.AddMessage("Warning: No textures found for material \"" + texList.material.name + "\".", PatStuff.ScreenLogType.Warning);
				--numAtlasesBuilt;
			}
			else
				BuildAtlas(texList);
		...
		PatStuff.ScreenLog.AddMessage(numAtlasesBuilt + " atlases generated.");
	...
	void ScanProjectFolder(ArrayList sprites)
	{
		...
		while (stack.Count > 0)
		{
			...
			{
				// Error
				PatStuff.ScreenLog.AddMessage("Could not access folder: \"" + dir + "\"", PatStuff.ScreenLogType.Error);
			}
		}
	}
		...
		for (int i = 0; i < msMap.sprites.Count; ++i)
		{
			...
			for (int j = 0; j < spr.SourceTextures.Length; ++j)
			{
				...
				else
				{
						...
						PatStuff.ScreenLog.AddMessage("Null texture reference detected on sprite \"" + ((Component)spr).name + "\"! Please verify that this was intentional.", PatStuff.ScreenLogType.Warning);
					#endif
				}
			}
		}
...
// 		atlas = (Texture2D) AssetDatabase.LoadAssetAtPath(projectRelativePath, typeof(Texture2D));
		   ...
		   texList.uvs[0].height * atlas.height != ((Texture2D)texList.trimmedTextures[0]).height)
		{
			...
			PatStuff.ScreenLog.AddMessage("WARNING: Textures were resized to fit onto atlas \"" + atlas.name + "\"!  To resolve this, assign some of your sprites a different material, or if possible, use a larger maximum texture size.", PatStuff.ScreenLogType.Warning);
		}
		...
		PatStuff.ScreenLog.AddMessage("Finished building atlas \"" + texList.material.name + "\"...");
	}
	...
	void SetFrameInfo(MaterialSpriteList msMap, AtlasTextureList texList)
	{
		...
		for (int i = 0; i < msMap.sprites.Count; ++i)
		{
			...
			for (int j = 0; j < spAg.SpriteFrames.Length; ++j)
			{
				...
				if (index < 0)
				{
					PatStuff.ScreenLog.AddMessage("Error assigning frame info: frame " + j + " on object \"" + ((Component)spAg).name + "\" not found in the texture list!", PatStuff.ScreenLogType.Error);
					continue;
				}
