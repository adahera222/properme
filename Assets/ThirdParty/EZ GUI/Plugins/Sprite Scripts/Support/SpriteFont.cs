...
public class SpriteFont
{
				...
				try
				{
					...
					switch (blockType)
					{
						...
						case (char)5:
							...
							break;
						default:
							ScreenLog.AddMessage("Unexpected block type " + (int)blockType);
							break;
					}
				}
	...
	void ReadCommonBlock(BinaryReader br, int blockSize)
	{
		...
		if (pages > 1)
			ScreenLog.AddMessage("Multiple pages/textures detected for font \"" + face + "\". only one font atlas is supported.");
	...
	int FindField(string label, string[] fields, int pos, bool logError)
	{
		...
		if (logError)
		{
			ScreenLog.AddMessage("Missing \"" + label + "\" field in font definition file \"" + fontDef.name + "\". Please check the file or re-create it.", ScreenLogType.Error);
			return pos;
		}
		else
			return -1;
	...
	void CommonParser(string line)
	{
		...
		if (int.Parse(vals[index + 1]) > 1)
			ScreenLog.AddMessage("Multiple pages/textures detected for font \"" + face + "\". only one font atlas is supported.", ScreenLogType.Error);
	}
	...
	void CharCountParser(string line)
	{
		...
		if (vals.Length < 2)
		{
			ScreenLog.AddMessage("Malformed \"chars count\" line in font definition file \"" + fontDef.name + "\". Please check the file or re-create it.", ScreenLogType.Error);
			return;
		}
