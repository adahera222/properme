...
public class SimpleSprite : SpriteRoot
{
	...
	protected override void Awake ()
	{
		...
		SetPixelDimensionsFromTexture ();

		Init ();
	}
	
	/// <summary>
	/// Sets the pixel dimensions from texture.
	/// </summary>
	public void SetPixelDimensionsFromTexture()
	{
		if (renderer != null && renderer.sharedMaterial != null && renderer.sharedMaterial.mainTexture != null) {
			Vector2 dimenstions = new Vector2 (renderer.sharedMaterial.mainTexture.width, renderer.sharedMaterial.mainTexture.height);
			
			SetLowerLeftPixel (new Vector2 (0, dimenstions.y));
			SetPixelDimensions (new Vector2 (dimenstions.x, dimenstions.y));
		}
	}
