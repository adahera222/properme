    ...
    public class Finger
    {
        ...
        internal void Update( FingerPhase newPhase, Vector2 newPos )
        {
            ...
            if( phase != newPhase )
            {
                ...
                {
                    ScreenLog.AddMessage( "Correcting bad FingerPhase transition (FingerPhase.Ended skipped)", ScreenLogType.Warning);
                ...
                {
                    ScreenLog.AddMessage( "Correcting bad FingerPhase transition (FingerPhase.Began skipped)", ScreenLogType.Warning);
                ...
                {
                    ScreenLog.AddMessage( "Invalid state FingerPhase transition from " + phase + " to " + newPhase + " - Skipping.", ScreenLogType.Warning);
                    return;
                }
            }
            else // same phase as before
            {
                ...
                {
                    ScreenLog.AddMessage( "Duplicated FingerPhase." + newPhase.ToString() + " - skipping.", ScreenLogType.Warning);
                    return;
                }
            }
