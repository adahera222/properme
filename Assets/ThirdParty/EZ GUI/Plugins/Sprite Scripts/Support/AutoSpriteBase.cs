...
public abstract class AutoSpriteBase : SpriteBase, ISpriteAggregator, ISpritePackable
{
	...
	public override Vector2 GetDefaultPixelSize(PathFromGUIDDelegate guid2Path, AssetLoaderDelegate loader)
	{
		...
		if(f == null)
		{
			ScreenLog.AddMessage("Sprite \"" + name + "\" does not seem to have been built to an atlas yet.", ScreenLogType.Warning);
			return Vector2.zero;
		}
	...
	public void PlayAnim(int index, int frame)
	{
		...
		{
			ScreenLog.AddMessage("ERROR: Animation index " + index + " is out of bounds!", ScreenLogType.Error);
			return;
		}
	...
	public void PlayAnim(string name, int frame)
	{
		...
		ScreenLog.AddMessage("ERROR: Animation \"" + name + "\" not found!", ScreenLogType.Error);
	}
