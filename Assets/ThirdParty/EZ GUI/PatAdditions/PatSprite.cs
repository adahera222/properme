...
/// </remarks>
[AddComponentMenu("Pat/PatSprite")]
public class PatSprite : AutoSpriteControlBase
{
	...
	//if true will automatically toggle on click, otherwise we will control it
	public bool toggleAutomatically = false;
		...
		if (Application.isPlaying)
		{
			...
			for (j = 0; j < states.Length; ++j)				
			{
				...
				for (i = 0; i < layers.Length; ++i)
				{
					if (layers[i] == null)
					{
						ScreenLog.AddMessage("A null layer sprite was encountered on control \"" + name + "\". Please fill in the layer reference, or remove the empty element.", ScreenLogType.Error);
						continue;
					}
	...
	static public PatSprite Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (PatSprite)go.AddComponent(typeof(PatSprite));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public PatSprite Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
