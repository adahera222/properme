		...
		get
		{
			if (s_Instance == null)
			{
				...
				if (s_Instance == null && Application.isEditor)
					ScreenLog.AddMessage("Could not locate a UIManager object. You have to have exactly one UIManager in the scene.", ScreenLogType.Error);
			}
...
// If we have a mouse under any possible circumstances, poll it:
	...
	{
		ScreenLog.AddMessage("The UIManager encountered a collider on object \"" + obj.name + "\" that does not not contain an IUIObject or derivative component.  Please double-check that this object has the correct layer and components assigned.", ScreenLogType.Warning);
	}
}
